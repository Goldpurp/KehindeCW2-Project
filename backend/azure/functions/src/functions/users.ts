...
const normalizeUser = (userId: string, body: Partial<UserRecord>): UserRecord => {
  ...
  return {
    ...
    uid: userId,
    email: body.email || '',
    displayName: body.displayName?.trim() || body.email?.split('@')[0] || 'kehindeScalableSolution user',
...
export async function signUp(request: HttpRequest): Promise<HttpResponseInit> {
  try {
    ...
    const user: UserRecord = {
      ...
      uid,
      email,
      displayName: body.displayName?.trim() || email.split('@')[0] || 'kehindeScalableSolution user',
      role: requestedRole,
